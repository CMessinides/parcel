...
export default class BundleGraph {
  ...
  traverseBundle<TContext>(
    ...
    return this._graph.filteredTraverse(
      ...
      visit,
      nullthrows(this._graph.getNode(bundle.id)),
      bundle.entryAssetId == null ? 'contains' : null
    );
  }
  ...
  traverseContents<TContext>(
    ...
    return this._graph.traverse(visit);
  }
  ...
  traverseBundles<TContext>(visit: GraphVisitor<Bundle, TContext>): ?TContext {
    return this._graph.traverse(
      mapVisitor(node => (node.type === 'bundle' ? node.value : null), visit),
      null,
      'bundle'
    );
  }
