...
export default class BundlerRunner {
  ...
  async bundle(graph: AssetGraph): Promise<InternalBundleGraph> {
    ...
    let bundleGraph = bundlerAssetGraphFromAssetGraph(graph);
    // $FlowFixMe
    let internalBundleGraph = new InternalBundleGraph(bundleGraph);
    await dumpGraphToGraphViz(bundleGraph, 'before_insert');
    ...
    await dumpGraphToGraphViz(bundleGraph, 'after_process');
  ...
  async addRuntimesToBundle(
    ...
    for (let {code, filePath, dependency} of runtimeAssets) {
      ...
      let subBundleGraph = new InternalBundleGraph(
        // $FlowFixMe
        bundlerAssetGraphFromAssetGraph(
...
function bundlerAssetGraphFromAssetGraph(
